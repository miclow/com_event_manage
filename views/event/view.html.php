	...
	public function display($tpl = null)
	{
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			...
			if($this->form->validate($formdata)){	
				...
				if(!empty($step) && !in_array($step, array('new','back')) && !empty($this->sid)){
		    	...
		    	if(!empty($this->state)){
					...
					if($ee != false ){
						$session->clear('renew');
						JFactory::getApplication()->enqueueMessage('Page one stored successfully');	
		...
		if($session->get('event_id', 'NULL') != 'NULL'){
			...
			$this->olduserdata = unserialize($this->progress->data);
			foreach ($this->pageKey as $value) {
					$data['onpage'] = $this->progress->onpage;
					if(array_key_exists($value, $this->olduserdata)){
						$data['data'][$value] = $this->olduserdata[$value];
						if($data['data']['page1']['start_date']){
							$data['data']['page1']['start_date'] = '';
						}
					}
			}
			$data['status'] = '1';
				...
				if(!isset($step)){		
				   ...
				   if(!in_array($step, $this->stepArr)){
				   		if($step != 'renew'){
							...
							$url = JRoute::_('index.php?option=com_event_manage&view=event&step='.$step.$sid);
							$app->redirect($url);
						}
					}
				}
