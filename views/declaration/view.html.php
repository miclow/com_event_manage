	...
	public function display($tpl = null)
	{
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {			
			...
			if($this->form->validate($formdata)){	
				...
				if($formdata['status'] == 'found'){
					...
					$data['Uid'] = $this->usercreate->id;
		    			...
		    			if($ee === 'update'){
							...
							$body .= "</tr>";
							$body .= "</table></div><br>";
							$body .= '<div>Thank & Regards</br></br>';
							$body .= 'Action Entertainment Insurance</div><br>';
							
					    	$mailData = array();
							$mailData['subject'] = "save come back later";
				...
				if($ee != false ){
					JFactory::getApplication()->enqueueMessage('Page five stored successfully');	
		...
		if($this->frontdata != '' && isset($this->frontdata->data['page4'])){
			if($this->frontdata->data['page1']['start_date']){					
				...
				$data['onpage']= '5';
				// $data['data']['page1'] = $this->frontdata->data['page1'];	
				// $data['data']['page2'] = $this->frontdata->data['page2'];
				// $data['data']['page3'] = $this->frontdata->data['page3'];
				// $data['data']['page4'] = $this->frontdata->data['page4'];
				// $data['data']['page5']['termconditions'] = '';
				// $data['data']['page5']['fsgread'] = '';
				foreach ($this->pageKey as $value) {
						if(array_key_exists($value, $this->frontdata->data)){
							$data['onpage'] = $this->frontdata->onpage;		
							if($value == 'page5'){
								$data['data']['page5']['termconditions'] = '';
								$data['data']['page5']['fsgread'] = '';
							}else{
								$data['data'][$value] = $this->frontdata->data[$value];
							}
						}
				}
				$data['status'] = '1';
				...
				JFactory::getApplication()->enqueueMessage('Policy start date should not be blank','error');	
				if(!$user_id){
					$callback = '';
				}else{
