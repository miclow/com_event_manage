	...
	<div class="form-body">		
		<form action="" method="post" enctype="multipart/form-data" id="page1" class="form-validate">
			...
			<div class="row-fluid top15">
				<div class="span12" style="margin-left:0";>
					<div class="control-group span4">
						<div class="control-label"><?php echo $this->form->getLabel('liailitycover'); ?></div>
						<div class="controls">
							<select aria-required="true" required="required" class="required" name="jform[liailitycover]" id="jform_liailitycover" aria-invalid="true">
								<option value="">Please Select</option>
								<?php 
								foreach ($lcover as $lcvalue) {
									$lcchek = '';
									if(!empty($getliailitycover)){
										if($lcvalue->price == $getliailitycover){
											$lcchek = 'selected="selected"';
										}else{
											$lcchek = '';
										}
									}
									echo '<option value="'.$lcvalue->price.'" '.$lcchek.'>'.$lcvalue->name.'</option>';
								}
								?>
							</select>
						</div>
