...
<script type="text/javascript">
	...
	function ajax_post(a,activity){
	    ...
	    hr.onreadystatechange = function() {
	    	if(hr.readyState == 4 && hr.status == 200) {
	    		...
	    		// alert(obj.extraField);
	    		dropdown.innerHTML = obj.extraField ;
	    		localStorage['myKey'] = JSON.stringify(obj.extraField);
	    		...
	    		if(obj.status == 'Unfinished'){
		            ...
		            var onpageinput = document.createElement("input");
	    				...
	    				window.location.href = referralurl+lstart+modal;
	...
	js(document).ready(function () {
		...
		js("#selectdropdown").html(JSON.parse(localStorage['myKey']));

		js("#new_extra_charge").click(function(){
			js("#allextrafield").attr("disabled", "disabled");
			js("#btl-input-user-check-name").val("");
			js("#btl-input-user-check-name").removeAttr("disabled");
			js("#btl-input-user-check-price").val("");
			js("#btl-input-user-check-price").removeAttr("disabled");
			js("#btl-input-user-check-des").val("");
			js("#btl-input-user-check-des").removeAttr("disabled");
		});

		js("#cancel_extra_charge").click(function(){
			js("#allextrafield").removeAttr("disabled", "disabled");
			js("#allextrafield").val();
			js("#btl-input-user-check-name").val("");
			js("#btl-input-user-check-name").attr("disabled", "disabled");
			js("#btl-input-user-check-price").val("");
			js("#btl-input-user-check-price").attr("disabled", "disabled");
			js("#btl-input-user-check-des").val("");
			js("#btl-input-user-check-des").attr("disabled", "disabled");
		});
		
		// js("#allextrafield").change(function(){
		js( document ).on( 'change', '#allextrafield', function(){
			// alert('yes');
			...
			var datasubmit = js("#allextrafield").serialize();
			// var datasubmit = js("#allextrafield").val();
			...
			js.ajax({
			   ...
			   url: 'index.php?option=com_event_manage&task=checkUserExtrafieldById',
			   data: datasubmit,
			   success: function(html){	
			   		obj = JSON.parse(html);		
			   		// alert(obj.name);
			   		if(obj.status == 'found'){	
			   			js("#btl-input-user-check-name").val(obj.name);
                        js("#btl-input-user-check-price").val(obj.price);
                        js("#btl-input-user-check-des").val(obj.description);
                        js("#btl-input-user-check-name1").val(obj.name);
                        js("#btl-input-user-check-price1").val(obj.price);
                        js("#btl-input-user-check-des1").val(obj.description);
			   		}else if(obj.status == 'notfound'){
			   			js("#btl-input-user-check-name").val();
                        js("#btl-input-user-check-price").val();
                        js("#btl-input-user-check-des").val();
                        js("#btl-input-user-check-name1").val();
                        js("#btl-input-user-check-price1").val();
                        js("#btl-input-user-check-des1").val();
			   		}
			   },
...
<div class="page1">
	<div class="form-title"> <h3>Users List Page </h3></div>
							...
							if(!empty($this->userdetail)){
								foreach ($this->userdetail as $value) {
									...
									<tr>
										...
										<td><?php 
											if(!empty($value->activity_status)){ 
...
<div id="openModalReffralThird" class="modalDialog">
	<div class="inner-div">
		...
		<div id="btl-check-user" class="btl-content-block">
			<form name="add-extra-charge" id="extrafieldform" enctype="multipart/form-data" class="btl-extrafieldform" action="index.php?option=com_event_manage&view=userslist" method="post">
				<div id="btl-check-user-in-process"></div>	
				<h3>Add Extra Field</h3>
				<div class="spacer"></div>
				<div class="btl-error" id="btl-check-user-error"></div>
				<div class="btl-field">
					<div class="selectdropdown" id="selectdropdown"></div>
					<div class="btl-input">
						Field Name: <input disabled="disabled" id="btl-input-user-check-name" required="true" type="text" name="jform[name]" /><br>
						Value/Price: <input disabled="disabled" id="btl-input-user-check-price" required="true" type="text" name="jform[price]" /><br>
						Description: <textarea disabled="disabled" id="btl-input-user-check-des" name="jform[description]" cols="40"></textarea>

						<input  id="btl-input-user-check-name1" required="true" type="hidden" name="jform[name1]" />
						<input id="btl-input-user-check-price1" required="true" type="hidden" name="jform[price1]" />
						<textarea id="btl-input-user-check-des1" style="display:none;" name="jform[description1]" cols="40"></textarea>
					</div>
				</div>
				<div class="btl-buttonsubmit" id="btl-buttonsubmit">						
					...
					<!-- <input type="hidden" name="jform[old_activity_status]" id="old_activity_status" value="" /> -->
					<?php if($this->orderno) { ?>  					
					...
					?>
						<input type="hidden" name="jform[orderid]" id="orderid" value="<?php echo $orderdetail['orderid']; ?>" />
						<input type="hidden" name="jform[onpage]" id="onpage" value="<?php echo $orderdetail['onpage']; ?>" />
					<?php } ?>
					...
					<input type="submit" name="jform[save]" id="btn-primary-apply" class="btn btn-primary" value="Apply" />
					<input type="submit" name="jform[savenew]" id="btn-primary-apply" class="btn btn-primary" value="Apply & continue" />
					<input type="submit" name="jform[save]" id="btn-primary-apply" class="btn btn-primary" value="Save as Copy" />
					<a href="javascript:;" title="New" id="new_extra_charge" class="btn btn-primary">New</a>
					<a href="#close" id="cancel_extra_charge" title="Close" class="btn btn-primary" style="margin-top:10px;">Cancel</a>
			...
			<form name="add-extra-charge" id="extrafieldform" enctype="multipart/form-data" class="btl-extrafieldform" action="index.php?option=com_event_manage&view=userslist" method="post">
				...
				<div class="btl-buttonsubmit" id="prem-btl-buttonsubmit">						
					...
					<?php } else{ 
						$orderdetail = $session->get('OrderDetail');
					?>
						<input type="hidden" name="jform[orderid]" id="prem-orderid" value="<?php echo $orderdetail['orderid']; ?>" />
						<input type="hidden" name="jform[onpage]" id="prem-onpage" value="<?php echo $orderdetail['onpage']; ?>" />
					<?php } ?>
					<input type="submit" name="jform[save]" class="btn btn-primary" value="save" />
					<?php echo JHtml::_('form.token');?>
				</div>
