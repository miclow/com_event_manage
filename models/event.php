...
class Event_manageModelEvent extends JModelLegacy
{
	...
	function setCreateNewUser($userdetail){
		...
		if(!$resgister){
			...
			$mailer->setSubject('New Account Details');
			$mailer->isHTML(true);
...
//============================================================+
	...
	// Old this function call for update user status 
	...
	// New this function call for update user status
	function savepdffilename($oid,$filename,$fieldname){

		if($fieldname == 'Policy'){
			$Field = 'policy_filename';
		}else if($fieldname == 'Certificate'){
			$Field = 'certificate_of_currency';
		}else if($fieldname == 'Questionnaire'){
			$Field = 'performsure_questionnaire_response';
		}
		$db = JFactory::getDBO();
		$query = "UPDATE `#__event_manage_session` SET `".$Field."`= '".$filename."' $query WHERE `id` = '".$oid."'";
	...
	/* Gernate XML File Function */
	function gernateXML($userdata){
		...
		$gernatedate = date('dmYHis');
		// $filename = $policy_no.'_'.$gernatedate.'.pdf';
		$filenamexml = $policy_no.'.xml';

		/* Page1 Data Start */
			if(!empty($data['page1']['liailitycover']) && (strpos($data['page1']['liailitycover'],'$') !== False)){
				$liailitycover1 = $data['page1']['liailitycover'];
			}else if(!empty($data['page1']['liailitycover'])){
				$liailitycover1 = '$'.number_format($data['page1']['liailitycover']);
			}else{
				$liailitycover1 = '$0.00';
			}
			$insurencename1 = 	$data['page1']['insurencename'];
			$activity1 = 	$activity;
			$perfomers1 = $data['page1']['perfomers'];
			$annualincome1 = 	$data['page1']['annualincome'];
			$postcode1 = 	$data['page1']['postcode'];
			$email1 = 	$data['page1']['email'];
			$startdate1 = $startdate;
			$publicliability1 = $liailitycover1;
		/* Page1 Data End */
			
		/* Page2 Data Start*/
			$TPremium = 0;
			$premium = round($data['page2']['premium'],2); 
			$TPremium += $premium;
			if(!empty($premium)){
			...
			$gstpremium = round($data['page2']['gstpremium'],2); 
			$TPremium += $gstpremium;
			if(!empty($gstpremium)){
				$gstpremium = '$'.number_format($gstpremium,2);
			}else{
				$gstpremium = '$0.00';
			}

			$gstfee = round($data['page2']['gstfee'],2);
			$TPremium += $gstfee; 
			if(!empty($gstfee)){
				$gstfee = '$'.number_format($gstfee,2);
			}else{
				$gstfee = '$0.00';
			}

			$stampduty = round($data['page2']['stampduty'],2);
			$TPremium += $stampduty;
			if(!empty($stampduty)){
				$stampduty = '$'.number_format($data['page2']['stampduty'],2);
			}else{ 
				$stampduty = '$0.00'; 
			}

			$brokerfee = round($data['page2']['brokerfee'],2);
			$TPremium += $brokerfee;
			if(!empty($brokerfee)){
				$brokerfee = '$'.number_format($data['page2']['brokerfee'],2);	
			}else{
				$brokerfee = '$0.00';	
			}
		

			$tp = number_format((float)$data['page2']['subtotal'], 2, '.', '');
			if(!empty($tp)){
				$subtotal = '$'.$tp;
			}else{
				$subtotal = '$0.00';
			...
			$commission = round($data['page2']['commission'],2); 
			if(!empty($commission)){
			...
			$gstcommission = round($data['page2']['gstcommission'],2);
			if(!empty($gstcommission)){
				$gstcommission = '$'.number_format($gstcommission,2);	
			}else{
				$gstcommission = '$0.00';
			}

			$TotalPremium = round($TPremium,2);
			if(!empty($TotalPremium)){
			...
			$premium2 = $premium;
			$AdministrationFee2 =  $brokerfee;
			$SubTotal2 = $subtotal;
			$GSTPremium2 = $gstpremium;
			$StampDuty2 = $stampduty;
			$AdministrationFeeGST2 = $gstfee;
			$TotalPremium2 = $TotalPremium;
			$commission2 = $commission;
			$gstcommission2 = $gstcommission;
		/* Page2 Data End */
		
		/* Page3 Data Start*/

			$selfpromoteQuestion = 'Do you intend to hire out a performance venue to self-promote or stage your own performance?';
			if($data['page3']['selfpromote'] == 'Y'){
				$selfpromote = 'YES';
			}else if($data['page3']['selfpromote'] == 'N'){
				$selfpromote = 'NO';
			...
			$harmlessorindemnityQuestion = 'Will you be signing any contracts that contain hold harmless or indemnity agreements?';
			if($data['page3']['harmlessorindemnity'] == 'Y'){
				$harmlessorindemnity = 'YES';
			}else if($data['page3']['harmlessorindemnity'] == 'N'){
				$harmlessorindemnity = 'NO'; 
			...
			$subcontractorsQuestion = 'Do you use subcontractors?';
			if($data['page3']['subcontractors'] == 'Y'){
				$subcontractors = 'YES';
			}else if($data['page3']['subcontractors'] == 'N'){
				$subcontractors = 'NO';
			...
			$refusedinsuranceQuestion = 'Have you previously been refused insurance or had your insurance cancelled by an insurer or have had special conditions, increased premiums or increased excesses imposed on any policy of insurance by an insurer?';
			if($data['page3']['refusedinsurance'] == 'Y'){
				$refusedinsurance = 'YES';
			}else if($data['page3']['refusedinsurance'] == 'N'){
				$refusedinsurance = 'NO';
			...
			$liabilityclaimQuestion = 'Have you suffered any public liability claims or have caused incidents that give rise to public liability claim?';
			if($data['page3']['liabilityclaim'] == 'Y'){
				$liabilityclaim = 'YES';
			}else if($data['page3']['liabilityclaim'] == 'N'){
				$liabilityclaim = 'NO';
			...
			$criminaloffenceQuestion = 'Have you been charged or convicted of a criminal offence (excluding driving convictions) in the last 10 years?';
			if($data['page3']['criminaloffence'] == 'Y'){
				$criminaloffence = 'YES';
			}else if($data['page3']['criminaloffence'] == 'N'){
				$criminaloffence = 'NO';
			...
			$dangerousactivitiesQuestion = 'Audience participation with use of fire, sporting, hazardous or dangerous activities';
			if($data['page3']['dangerousactivities'] == 'Y'){
				$dangerousactivities = 'YES';
			}else if($data['page3']['dangerousactivities'] == 'N'){
				$dangerousactivities = 'NO';
			...
			$pyrotechnicsQuestion = 'Fireworks or pyrotechnics';
			if($data['page3']['pyrotechnics'] == 'Y'){
				$pyrotechnics = 'YES';
			}else if($data['page3']['pyrotechnics'] == 'N'){
				$pyrotechnics = 'NO';
			...
			$animalsQuestion = 'Use of animals';
			if($data['page3']['animals'] == 'Y'){
				$animals = 'YES';
			}else if($data['page3']['animals'] == 'N'){
				$animals = 'NO';
			...
			$amusementridesQuestion = 'Use of amusement rides or devices';
			if($data['page3']['amusementrides'] == 'Y'){
				$amusementrides = 'YES';
			}else if($data['page3']['amusementrides'] == 'N'){
				$amusementrides = 'NO';
			...
			$workshopsQuestion = 'Running of workshops';
			if($data['page3']['workshops'] == 'Y'){
				$workshops = 'YES';
			}else if($data['page3']['workshops'] == 'N'){
				$workshops = 'NO';
			...
			$northamericaQuestion = 'Any activities conducted in North America';
			if($data['page3']['northamerica'] == 'Y'){
				$northamerica = 'YES';
			}else if($data['page3']['northamerica'] == 'N'){
				$northamerica = 'NO';
		...
		/* Page4 Data Start*/
			$FirstName4 = $data['page4']['firstname'];
			$LastName4 = $data['page4']['lastname'];
			$insured4 = $data['page4']['insuredname'];
			$addressline1 = $data['page4']['streetaddress1'];
			$addressline2 = $data['page4']['streetaddress2'];
			$state4 = $state;
			$suburb4 = $data['page4']['suburb'];
			$postcode4 = $data['page4']['postcode'];
			$phone4 = $data['page4']['phone'];
			if($data['page4']['sameaddress'][0] == 'Y'){
				$sameaddress4 = 'Yes';
			}else{
				$sameaddress4 = 'No';
			}	
			$billingaddress1 = $data['page4']['billingaddress1'];
			$billingaddress2 = $data['page4']['billingaddress2'];
			$billingsuburb4 = $data['page4']['billingsuburb'];
			$billingstate4 = $billingstate;
			$billingpostcode4 = $data['page4']['billingpostcode'];
			$email4 = $data['page4']['email'];
		/* Page4 Data End */
		
		/* Page5 Data Start*/
			$termconditionsQuestion = 'You acknowledge that you are entering into a legally binding contract for the provision of insurance cover and will settle all amounts due in accordance with the Terms and Conditions';
			if($data['page5']['termconditions']){
				$termconditions5 = 'Yes';
			}else{
				$termconditions5 = 'No';
			}

			$fsgreadQuestiuon = 'You acknowledge that you have read the Financial Services Guide (FSG)';
			if($data['page5']['fsgread']){
				$fsgread5 = 'Yes';
			}else{
				$fsgread5 = 'No';
			}
		...
		/* Page6 Data Start*/
			...
			if($data['page6']['creditcard'] == 'Y'){
				$PaymentByCreditCard = 'Yes';
			}else{
				$PaymentByCreditCard = 'No';
			}

			$startdate6 = date("d/m/Y",strtotime($data['page6']['startdate']));
			
			$publicliability6 = $publicliability;

			if(!empty($data['page6']['totalpremium'])){
				$TotalPremium6 = '$'.number_format(round($data['page6']['totalpremium'],2),2);	
			}else{
				$TotalPremium6 = '$0.00';
			}

			if(!empty($data['page6']['subtotal'])){
				$subtotal6 = '$'.number_format(round($data['page6']['subtotal'],2),2);	
			}else{
				$subtotal6 = '$0.00';
			}

			if(!empty($data['page6']['orderTotal'])){
				$orderTotal6 = '$'.number_format(round($data['page6']['orderTotal'],2),2);	
			}else{
				$orderTotal6 = '$0.00';
			}

			if(!empty($data['page6']['creditcardfee'])){
				$creditcardfee6 = '$'.number_format(round($data['page6']['creditcardfee'],2),2);	
			}else{
				$creditcardfee6 = '$0.00';
			}
        ...
        $policyname = $domtree->createAttribute("policy_number");
		$xmlRoot->appendChild($policyname);

		/*create attribute value node*/
		$policynumber = $domtree->createTextNode($policy_no);
		$policyname->appendChild($policynumber);

        /* append it to the document created */
        $xmlRoot = $domtree->appendChild($xmlRoot);

        /* Page1 */
	        $requirementsTrack = $domtree->createElement("Requirements");
	        $requirementsTrack = $xmlRoot->appendChild($requirementsTrack);

	        /* you should enclose the following two lines in a cicle */
	        $requirementsTrack->appendChild($domtree->createElement('NameOfInsured',$insurencename1));
	        $requirementsTrack->appendChild($domtree->createElement('Activity',$activity1));
	        $requirementsTrack->appendChild($domtree->createElement('NumberOfPerformers',$perfomers1));
	        $requirementsTrack->appendChild($domtree->createElement('EstimatedAnnualIncome',$annualincome1));
	        $requirementsTrack->appendChild($domtree->createElement('Postcode',$postcode1));
	        $requirementsTrack->appendChild($domtree->createElement('Email',$email1));
	        $requirementsTrack->appendChild($domtree->createElement('Startdate',$startdate1));
	        $requirementsTrack->appendChild($domtree->createElement('PublicLiailityCover',$publicliability1));
	    /* Page1 End Here */
        
        /* Page2 */
	        $quotationTrack = $domtree->createElement("Quotation");
	        $quotationTrack = $xmlRoot->appendChild($quotationTrack);

	        $quotationTrack->appendChild($domtree->createElement('NameOfInsured',$insurencename1));
	        $quotationTrack->appendChild($domtree->createElement('Activity',$activity1));
	        $quotationTrack->appendChild($domtree->createElement('NumberOfPerformers',$perfomers1));
	        $quotationTrack->appendChild($domtree->createElement('EstimatedAnnualIncome',$annualincome1));
	        $quotationTrack->appendChild($domtree->createElement('Email',$email1));
	        $quotationTrack->appendChild($domtree->createElement('Startdate',$startdate1));
	        $quotationTrack->appendChild($domtree->createElement('PublicLiailityCover',$publicliability1));
	        $quotationTrack->appendChild($domtree->createElement('Premium',$premium2));
	        $quotationTrack->appendChild($domtree->createElement('BrokerFee',$AdministrationFee2));
	        $quotationTrack->appendChild($domtree->createElement('Subtotal',$SubTotal2));
	        $quotationTrack->appendChild($domtree->createElement('GSTPremium',$GSTPremium2));
	        $quotationTrack->appendChild($domtree->createElement('Stampduty',$StampDuty2));
	        $quotationTrack->appendChild($domtree->createElement('AdministrationFeeGST',$AdministrationFeeGST2));
	        $quotationTrack->appendChild($domtree->createElement('TotalPremium',$TotalPremium2));
	        $quotationTrack->appendChild($domtree->createElement('Commission',$commission2));
	        $quotationTrack->appendChild($domtree->createElement('GSTCommission',$gstcommission2));
        /* Page2 End Here */

        /* Page3 */
	        $proposalTrack = $domtree->createElement("Proposal");
	        $proposalTrack = $xmlRoot->appendChild($proposalTrack);

	        $proposalTrack->appendChild($domtree->createElement('Question',$selfpromoteQuestion));
	        $proposalTrack->appendChild($domtree->createElement('Answer',$selfpromote));
	        $proposalTrack->appendChild($domtree->createElement('Question',$harmlessorindemnityQuestion));
	        $proposalTrack->appendChild($domtree->createElement('Answer',$harmlessorindemnity));
	        $proposalTrack->appendChild($domtree->createElement('Question',$subcontractorsQuestion));
	        $proposalTrack->appendChild($domtree->createElement('Answer',$subcontractors));
	        $proposalTrack->appendChild($domtree->createElement('Question',$refusedinsuranceQuestion));
	        $proposalTrack->appendChild($domtree->createElement('Answer',$refusedinsurance));
	        $proposalTrack->appendChild($domtree->createElement('Question',$liabilityclaimQuestion));
	        $proposalTrack->appendChild($domtree->createElement('Answer',$liabilityclaim));
	        $proposalTrack->appendChild($domtree->createElement('Question',$criminaloffenceQuestion));
	        $proposalTrack->appendChild($domtree->createElement('Answer',$criminaloffence));
	        $proposalTrack->appendChild($domtree->createElement('Question',$dangerousactivitiesQuestion));
	        $proposalTrack->appendChild($domtree->createElement('Answer',$dangerousactivities));
	        $proposalTrack->appendChild($domtree->createElement('Question',$pyrotechnicsQuestion));
	        $proposalTrack->appendChild($domtree->createElement('Answer',$pyrotechnics));
	        $proposalTrack->appendChild($domtree->createElement('Question',$animalsQuestion));
	        $proposalTrack->appendChild($domtree->createElement('Answer',$animals));
	        $proposalTrack->appendChild($domtree->createElement('Question',$amusementridesQuestion));
	        $proposalTrack->appendChild($domtree->createElement('Answer',$amusementrides));
	        $proposalTrack->appendChild($domtree->createElement('Question',$workshopsQuestion));
	        $proposalTrack->appendChild($domtree->createElement('Answer',$workshops));
	        $proposalTrack->appendChild($domtree->createElement('Question',$northamericaQuestion));
	        $proposalTrack->appendChild($domtree->createElement('Answer',$northamerica));
        /* Page3 End Here */
        
        /* Page4 */
	        $addressTrack = $domtree->createElement("Address");
	        $addressTrack = $xmlRoot->appendChild($addressTrack);

	        $addressTrack->appendChild($domtree->createElement('FirstName',$FirstName4));
	        $addressTrack->appendChild($domtree->createElement('LastName',$LastName4));
	        $addressTrack->appendChild($domtree->createElement('NameOfInsured',$insured4));
	        $addressTrack->appendChild($domtree->createElement('StreetAddressOne',$addressline1));
	        $addressTrack->appendChild($domtree->createElement('StreetAddressTwo',$addressline2));
	        $addressTrack->appendChild($domtree->createElement('Suburb',$suburb4));
	        $addressTrack->appendChild($domtree->createElement('StateTerritory',$state4));
	        $addressTrack->appendChild($domtree->createElement('Postcode',$postcode4));
	        $addressTrack->appendChild($domtree->createElement('PhoneNumber',$phone4));
	        $addressTrack->appendChild($domtree->createElement('BillingAddressSameAsPolicyAddress',$sameaddress4));
	        $addressTrack->appendChild($domtree->createElement('BillingStreetAddressOne',$billingaddress1));
	        $addressTrack->appendChild($domtree->createElement('BillingStreetAddressTwo',$billingaddress2));
	        $addressTrack->appendChild($domtree->createElement('BillingSuburb',$billingsuburb4));
	        $addressTrack->appendChild($domtree->createElement('BillingStateTerritory',$billingstate4));
	        $addressTrack->appendChild($domtree->createElement('BillingPostcode',$billingpostcode4));
        /* Page4 End Here */
        
        /* Page5 */
	        $declarationTrack = $domtree->createElement("Declaration");
	        $declarationTrack = $xmlRoot->appendChild($declarationTrack);

	        $declarationTrack->appendChild($domtree->createElement('Question',$termconditionsQuestion));
	        $declarationTrack->appendChild($domtree->createElement('Answer',$termconditions5));
	        $declarationTrack->appendChild($domtree->createElement('Question',$fsgreadQuestiuon));
	        $declarationTrack->appendChild($domtree->createElement('Answer',$fsgread5));
        /* Page5 End Here */
        
        /* Page6 */
	        $checkoutTrack = $domtree->createElement("Checkout");
	        $checkoutTrack = $xmlRoot->appendChild($checkoutTrack);

	        $checkoutTrack->appendChild($domtree->createElement('PolicyNumber',$policy_no));
	        $checkoutTrack->appendChild($domtree->createElement('Startdate',$startdate6));
	        $checkoutTrack->appendChild($domtree->createElement('PublicLiabilityCover',$publicliability6));
	        $checkoutTrack->appendChild($domtree->createElement('TotalPremium',$TotalPremium6));
	        if(!empty($data['page6']['extrachreges'])){
		        foreach ($data['page6']['extrachreges'] as $key => $value) {
					if(!empty($value) && (strpos($value,'$') !== False)){
						$extvalue = number_format($value,2);
					}else if(!empty($value)){
						$extvalue = '$'.number_format($value,2);
					}
					$checkoutTrack->appendChild($domtree->createElement(ucfirst($key),$extvalue));
				}
			}
	        $checkoutTrack->appendChild($domtree->createElement('Subtotal',$subtotal6));
	        $checkoutTrack->appendChild($domtree->createElement('CreditCardFee',$creditcardfee6));
	        $checkoutTrack->appendChild($domtree->createElement('OrderTotal',$orderTotal6));
	        $checkoutTrack->appendChild($domtree->createElement('PaymentByCreditCard',$PaymentByCreditCard));
        /* Page6 End Here */

        $filepath = 'components/xmlpolicy/'.$filenamexml;
		$path = JPATH_ROOT.'/'.$filepath;
        // echo $domtree->saveXML();
        if($domtree->save($path)){
        	return true;
        }
	
	}

	function getDistinctExtraField(){

		$db = JFactory::getDBO();
		$query = "SELECT id,name,price,description,referral_page_no FROM  #__event_manage_term WHERE activity = 'Excharge'" ;
		$query .= " AND name NOT IN ('premium','brokerfee') group by name,referral_page_no";
		// $query = "SELECT id,name,price,description FROM  #__event_manage_term WHERE activity = 'Excharge'" ;
		// $query .= " AND name NOT IN ('premium','brokerfee') AND referral_page_no=".$rfno." group by name,referral_page_no";
	...
	/* Check user exit or not in system */
	function getcheckUserName($email){
